function [Estimatedcoordinates]=COG(Target_GridMaxArea, Indices, flag)
...
for i=1:m
    if Target_GridMaxArea(i,:)==[0 0 0 0]
        ...
        %Target_GridMaxArea2 is going to contain the MAX values for the
        %overlapping areas without zeros in the rows
    ...
    if (isempty(find(Indices==i))==1)
        ...
        for j=1:m
            if i==Target_GridMaxArea2(j,1) %Search for a target node in Target_GridMaxArea
                ForComputingCx(count+1,:)=[Target_GridMaxArea2(count+1,2)]; %Note: row 2 is for x, row 3 for y (remember that z=0)
...
%Plot estimated coordinates, we use the flag in order to change the design
%of the marker for the computation of the Estimatedcoordinates (we may use
%this for example, when we use APIT in a second iteration, or coordinates
%obtained after multilateration)
if flag==0
...
elseif flag==1
    plot(Estimatedcoordinates(:,2),Estimatedcoordinates(:,3),'v','MarkerEdgeColor','k','MarkerFaceColor','b');
    labels = num2str(Estimatedcoordinates(:,1));
    %For labelling each sensor
    text(Estimatedcoordinates(:,2)+.06,Estimatedcoordinates(:,3),labels,'Color','r');
end
