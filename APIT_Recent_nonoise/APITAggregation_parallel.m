...
%APITAggregation_parallel computes the overlapping of positive and negative
...
%Compute which points of the grid are inside or outside of the anchors that
%define Triangles outside of a particular node.
%Compute which points of the grid are inside or outside of the anchors that
%define Triangles inside of a particular node.

for t=1:w %size of H
%RefpointAoutside=[i X(TableNodesOutside(j,2),1) X(TableNodesOutside(j,2),2) 0]
%RefpointBoutside=[i X(TableNodesOutside(j,3),1) X(TableNodesOutside(j,3),2) 0]
%RefpointCoutside=[i X(TableNodesOutside(j,4),1) X(TableNodesOutside(j,4),2) 0]
    if(isempty(RefpointAoutside)==0)
        ...
        fprintf(1,'--Checking GRID POINT H=[%f %f] for outside cases in node %d\n',Hrepo(1,1),Hrepo(1,2),i); %Only for reference we choose the first row
    end
...
%RefpointCoutside=[i X(TableNodesInside(j,4),1) X(TableNodesInside(j,4),2) 0]
    ...
    fprintf(1,'--Checking GRID POINT H=[%f %f] for inside cases %d\n',Hrepi(1,1),Hrepi(1,2),i);
    Hrepo=[];
    Hrepi=[];
end
...
if(isempty(RefpointAoutside)==0)
    while(indiceinferioro <= m) 
                ...
                for l=indiceinferioro+1:m %size Insidethetriangleo
                    %Format of Negativeareas=Negativeareas[TargetNode Point_of_the_grid Counter]
                    temp1o=[];
                    temp2o=[];
...
while(indiceinferiori <= r)
           ...
           if temp2i==1
            ...
            for l=indiceinferiori+1:r %size of Insidethetrianglei
                   ...
                   %Format of Positiveareas=Positiveareas[TargetNode Point_of_the_grid Counter]
...
%(sum) the values
if(isempty(RefpointAoutside)==0) 
%Do the procedure of considering positive and negative areas when we have the two arrays, if we do not have TableNodesOutside we do not have Negativeareas and therefore we can not run the following lines
    for j=1:m %size of Positiveareas
        no_reppos=0;
        Nodetosearch=Positiveareas(j,1); %Nodetosearch contains the node to be searched in Negativeareas just to save time
        Rangetosearch=find(Negativeareas(:,1)==Nodetosearch);
        [m_rangetosearch n_rangetosearch]=size(Rangetosearch);
        %m_rangetosearch=size of search
        for k=Rangetosearch(1):Rangetosearch(m_rangetosearch)      %TargetNode          %Gridpoint[x y z]
        ...
        if no_reppos==0
                Doespositiveexist=1;
                %Positiveareas_d will contain elements which are not
                %repetititve
    ...
    for j=1:o %size of Negativeareas
        no_reppos=0;
        Nodetosearch=Negativeareas(j,1); %Nodetosearch contains the node to be searched in Negativeareas just to save time
        Rangetosearch=find(Positiveareas(:,1)==Nodetosearch);
        [m_rangetosearch n_rangetosearch]=size(Rangetosearch);
        %size of search
        for k=Rangetosearch(1):Rangetosearch(m_rangetosearch)      %TargetNode          %Gridpoint[x y z]
...
letter1i='TargetGridMaxArea'
letter2i='.mat'
TARGETAREA=strcat(letter1i,Name_file,letter2i); %It concatenates: TargetGridMaxArea(Name_file).mat
%Saves the Target_GridMaxArea matrix in TargetGridMaxArea.mat file
save ("-text", TARGETAREA, "Target_GridMaxArea");
