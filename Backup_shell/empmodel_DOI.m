...
for i=1:((M+N)*360)
    if i==1 | mod(i,360)==1
        if i==360 %This belongs truly to i=359 (since i begins at 1), thus it has to change according to irregularity
    ...
    else %Ki, for i~=1 & i~=359 or ~ multiples (e.g. 359 in the 2nd cycle, etc).....
	if mod(i,2)==0
    	    K(i,:)=[Num_node K(i-1,2)+(RAND(i,1)*DOIv) Grades];
        else
	    K(i,:)=[Num_node K(i-1,2)-(RAND(i,1)*DOIv) Grades]; %We vary the irregularity by interchanging signs.
	end
	Grades=Grades+1;
    end
end
