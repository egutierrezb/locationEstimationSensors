...
%drawtriangles_parallel computes the tables of RSS for ONLY ONE targetNode and its
...
% TableNodesOutsidetempo is the TableNodesoutsidetemp without repetitions but
% that may contains zeros
% TableNodesOutsidetemp is the first table of which nodes are outside with
% possible repetitions
% TableNodesOutside is the final table obtained from the entire process
if Warningflag_impossibleest==1; %Means that no nodes were found to be estimated
    fprintf(1,'--The node can not be estimated!');
    Endprogram=1;
    return;
else
    Endprogram=0;
    ...
    if (isempty(find(Nodeswecanestimate==i))==1) % if (1) means that the node is not found in Nodeswecanestimate and therefore is part of the Nodeswecannotestimate
            ...
            Nodeswecannotestimate=[]; % if i is  found in Nodeswecanestimate, then Nodeswecannotestimate is emptuy
    end
    ...
    %We are dealing with only one node.
    %else
