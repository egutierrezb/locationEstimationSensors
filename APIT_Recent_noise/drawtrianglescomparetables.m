function [TableNodesOutside, TableNodesOutsidetempo, TableNodesInside, Endprogram]=drawtrianglescomparetables(X,Audibleanchors,RSS_noise,Neighborhood,Indices)
                ...
                for SetofAnchors=1:u %size of CombinationTriangles
                            ...
                            if(isempty(Neighborhood_shared)==0)
                                ...
                                for neigh=1:nl
                                 ...
                                 %RSS_noise(Match2,:)
                                 %RSS_noise(Match3,:)
                                 if(CombinationTriangles(SetofAnchors,1)==0 | CombinationTriangles(SetofAnchors,2)==0 | CombinationTriangles(SetofAnchors,3)==0) %<- da 0 pero de donde?
                                        BUSQUEDA=find(Audibleanchors(:,1)==i);
                                        [m n]=size(BUSQUEDA);
                                        Audibleanchors(BUSQUEDA(1):BUSQUEDA(m),2)
                                 end
...
if Warningflag_impossibleest==1; %Means that no nodes were found to be estimated
    ...
    %Just to not continue with the remaining of the program, and used for
    %comparison of the error tables
    TableNodesOutside=[];
