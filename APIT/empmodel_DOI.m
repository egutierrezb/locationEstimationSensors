function [Pij, TrueDist, RSS_noise, dth_RSS_Neighborhood]=empmodel_DOI(X)
global M N DOIv
    ...
    for j=1:(M+N)
        if i~=j
            ...
            if (X(i,2)-X(j,2))> 0 & (X(i,1)-X(j,1))> 0 %Ist Q
                m(cont,:)=[i (X(i,2)-X(j,2))/(X(i,1)-X(j,1)) 1 j];
            elseif (X(i,2)-X(j,2))> 0 & (X(i,1)-X(j,1))< 0 %2nd Q
                m(cont,:)=[i (X(i,2)-X(j,2))/(X(i,1)-X(j,1)) 2 j];
            elseif (X(i,2)-X(j,2))< 0 & (X(i,1)-X(j,1))< 0 %3rd Q
                m(cont,:)=[i (X(i,2)-X(j,2))/(X(i,1)-X(j,1)) 3 j];
            elseif (X(i,2)-X(j,2))< 0 & (X(i,1)-X(j,1))> 0 %4th Q
                m(cont,:)=[i (X(i,2)-X(j,2))/(X(i,1)-X(j,1)) 4 j];
            end
                cont=cont+1;
        end
    end
...
Pij=10*np*log10(TrueDist.*(1/do));
[m n]=size(TrueDist);
cont=1;
for i=1:m
    for j=1:m
        if TrueDist(i,j)<=1 %if the distance / two sensors is equal or less that 1 m
