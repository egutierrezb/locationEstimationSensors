function [InsideorOutside, TableNodesOutside, TableNodesInside, TableNodesOutsidetempo, Nodeswecannotestimate]=drawtriangles_noise(X,Audibleanchors,RSS_noise,Neighborhood,Indices)
...
for i=1:(M+N) %M+N
    ...
    %anchor node, it is a target node), also the i node has to be included
    ...
    if (isempty(find(Indices==i))==1) & (isempty(find(Neighborhood(:,1)==i))==0) 
        ...
        if Anchorscounter>=3
            ...
            for SetofAnchors=1:u %size of CombinationTriangles
                 ...
                 %Format of MatrixRSSTargetNode=[TargetNode Anchor RSS]  
                                                                                      %Pij(i,Anchor)
                                                                                      %Pij(i,CombinationTriangles(SetofAnchors,1)
                 Match1=find(RSS_noise(:,1)==i & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,1)); %Finds the row for the RSS to be used         
                 Match2=find(RSS_noise(:,1)==i & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,2)); %Finds the row for the RSS to be used         
                 Match3=find(RSS_noise(:,1)==i & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,3)); %Finds the row for the RSS to be used         
                 ...
                 for d=Index(marker_rowneighbori):Index(marker_rowneighbore) %Range of Neighborhood
                            ...
                            %Format of MatrixRSSNeighborhood=MatrixRSSNeighborhood=[TargetNode Neighbornode Anchor RSS]
                                                                                                                         %Pij(CombinationTriangles(SetofAnchors,1),Neighborhood(d,2))
                             %What would happen if we take the viceversa
                             %relationship? RSS_noise(:,1)=CombinationTriangles...
                             %RSS_noise(:,2)=Neighborhood we may
                             %have a different result since the noise is
                             %different for every node and is the
                             %connection is not simmetric as we had it
                             %before when no noise was considered (i.e.
                             %Pij=Pji)
                             Match1=find(RSS_noise(:,1)==Neighborhood(d,2) & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,1)); %Finds the row for the RSS to be used         
                             Match2=find(RSS_noise(:,1)==Neighborhood(d,2) & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,2)); %Finds the row for the RSS to be used         
                             Match3=find(RSS_noise(:,1)==Neighborhood(d,2) & RSS_noise(:,2)==CombinationTriangles(SetofAnchors,3)); %Finds the row for the RSS to be used         
                             %-> problema entre neighbor y el anchor es
                             %el mismo, cosa que en practica no existe, por
                             %lo tanto Matchx tendria un nulo.
                             if (isempty(Match1)==0 & isempty(Match2)==0 & isempty(Match3)==0)
                                 ...
                                 MatrixRSSNeighborhood(temp3,:)=[i Neighborhood(d,2) CombinationTriangles(SetofAnchors,1) RSS_noise(Match1,3)]; %Obtain RSS between the anchor and the neigbor
                                 MatrixRSSNeighborhood(temp3+1,:)=[i Neighborhood(d,2) CombinationTriangles(SetofAnchors,2) RSS_noise(Match2,3)];
                                 MatrixRSSNeighborhood(temp3+2,:)=[i Neighborhood(d,2) CombinationTriangles(SetofAnchors,3) RSS_noise(Match3,3)];
                                 temp3=temp3+3;
               ...
               %-----------------------------------------------------------
                if(aux3==Number_neighbors)
                 ...
                 Number_neighbors=0;
                 aux3=0;
                 %We may increase the pointers of the table of the
                 %target
...
%We may eliminate all the rows for which we can not estimate the position
%of the corresponding nodes, if there are nodes in nodeswecannotestimate,
%means that we can estimate the positions of all the nodes and we just
%compy the temp array to the definite array.
